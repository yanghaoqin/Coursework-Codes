...
public class SLinkedList<E extends DeepClone<E>> implements Iterable<E>
{
	...
	/*
 	...
 	{
 		/**** ADD CODE HERE ****/
 		if(this.isEmpty()) return new SLinkedList<E>();
 		
 		SLinkedList<E> deepCopy = new SLinkedList<E>();		// creates a new linked list to store deep copy

 		for(E cur: this) {									// Java enhanced for loop
 			deepCopy.addLast(cur.deepClone());				// add the element's deep clone copy to the 'deepCopy' SLL
 		}
